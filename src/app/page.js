...
export default function Home() {
  ...
  // 3. Efek untuk selalu mengupdate posisi mouse saat kursor bergerak
  useEffect(() => {
      ...
      <div className="pt-6 px-4 md:px-8 max-w-[1400px] mx-auto relative z-50">
        <nav className="flex items-center justify-between px-6 py-4 rounded-2xl bg-white/5 backdrop-blur-md border border-white/10 shadow-2xl relative">
          ...
          <div className="flex items-center gap-2 font-bold text-xl text-white italic relative z-10 w-1/4">
            <span>Vinto<span className="text-green-500">.</span></span>
          </div>

          {/* Menu Links (Diposisikan mutlak di tengah secara matematis) */}
          <div className="hidden lg:flex gap-8 text-sm font-medium text-gray-300 absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-max">
            <a href="#" className="text-white hover:text-green-400 transition-colors">{t.nav[0]}</a>
            <a href="#" className="hover:text-green-400 transition-colors">{t.nav[1]}</a>
            ...
            <a href="/blog" className="hover:text-green-400 transition-colors">{t.nav[5]}</a>
          </div>
        ...
        <div className="bg-white/5 backdrop-blur-lg border border-white/10 rounded-3xl p-8 md:p-12 lg:p-16 shadow-2xl relative overflow-hidden">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20 items-center">
            ...
            <div className="relative h-[400px] lg:h-[500px] w-full rounded-2xl overflow-hidden border border-white/10 shadow-lg group">
              {/* FOTO SEKARANG AKTIF DI SINI */}
               <Image 
                 src="/images/vinto-about.png" // Jalur ke file yang baru kamu simpan
                 alt="Vinto Working" 
                 fill // Mengisi seluruh wadah induk
                 className="object-cover group-hover:scale-105 transition-transform duration-700" // object-cover memastikan foto "fit" tanpa distorsi
               /> 
            </div>

            {/* Kanan: Teks About Me */}
            <div className="flex flex-col justify-center">
              {/* PERBAIKAN: Penambahan py-2 dan leading-normal agar text gradient tidak terpotong */}
